...
namespace KH.DAL
{
	...
	public partial class NewsDAL
	{
		...
		/// 增加一条数据
		/// </summary>
		public long Add(KH.Model.News model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into T_News(");
			strSql.Append("Title,Body,PostDateTime,CategoryId)");
			...
			parameters[2].Value = model.PostDateTime;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(KH.Model.News model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public KH.Model.News GetModel(long Id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Id,Title,Body,PostDateTime,CategoryId from T_News ");
			strSql.Append(" where Id=@Id");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.BigInt)
			};
			...
			KH.Model.News model=new KH.Model.News();
		...
		/// 得到一个对象实体
		/// </summary>
		public KH.Model.News DataRowToModel(DataRow row)
		{
			KH.Model.News model=new KH.Model.News();
			if (row != null)
			{
				...
				if(row["PostDateTime"]!=null && row["PostDateTime"].ToString()!="")
				{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Id,Title,Body,PostDateTime,CategoryId ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" Id,Title,Body,PostDateTime,CategoryId ");
			strSql.Append(" FROM T_News ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
